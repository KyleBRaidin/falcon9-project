...
def get_pie_chart(entered_site):
    if entered_site == 'ALL':
        ...
        site_data = spacex_df[spacex_df['Launch Site'] == entered_site].groupby(['Launch Site', 'class']).size().reset_index(name='class count')
        fig = px.pie(
            site_data,
            names = 'class',
            values = 'class count',
...
def update_scatter_plot(entered_site, payload):
    payload_data = spacex_df[(spacex_df['Payload Mass (kg)']>=payload[0]) & (spacex_df['Payload Mass (kg)']<=payload[1])]
    ...
    if entered_site == 'ALL':
        ...
        site_data = payload_data[payload_data['Launch Site']==entered_site]
        fig = px.scatter(
            site_data,
            x='Payload Mass (kg)',
            y='class',
